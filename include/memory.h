...
fc_error_t __memset(slice_t ptr, u8 c);
fc_error_t __memcpy(slice_t dst, slice_t src);
...
/* This is preventing mechanisim to avoid clangs automatic memset calls because those functions collide */
#define memset(ptr, c) __memset(ptr, c)
#define memcpy(dst, src) __memcpy(dst, src)
