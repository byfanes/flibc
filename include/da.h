...
#define da(type) const CONCAT(da_,type)

#define da_reserve(p_da, amount) __da_reserve((da), (amount), sizeof((da)->items[0]))
fc_error_t __da_reserve(const void* da, u32 amount, u32 el_size);

#define da_reserve_if(p_da, amount) __da_reserve((da), (amount), sizeof((da)->items[0]))
