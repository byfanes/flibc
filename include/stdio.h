...
error_t __fwrite(file_t* file, void* sl, usz el_size);
#define fwrite(file, sl) __fwrite((file), ptr_meta(sl))
error_t __fread(file_t* file, void* buf, usz el_size, u32* _Nullable read_count);
#define fread(file, buf, read_count) __fread((file), ptr_meta(buf), (read_count))

error_t fsetpos(file_t* file, usz pos);
error_t fgetpos(file_t* file, usz* _Nullable pos);

error_t ftell(file_t* file, usz* out);
error_t fseek(file_t* file, usz off, seek_type_t type, usz* _Nullable out);
error_t rewind(file_t* file);

error_t __fputs(file_t* file, void* sl, usz el_size);
#define fputs(file, sl) __fputs((file), ptr_meta(sl))
error_t fputc(file_t* file, uint8_t c);

error_t sprintf(sl_u8_t buf, usz* _Nullable wrote_count, const char* fmt, ...);
error_t sprintf_sl(sl_u8_t buf, usz* _Nullable wrote_count, sl_u8_t fmt, ...);

error_t fprintf(file_t* file, const char* fmt, ...);
error_t fprintf_sl(file_t* file, sl_u8_t fmt, ...);

error_t vsprintf(sl_u8_t buf, usz* _Nullable wrote_count, const char* fmt, va_list ap);
error_t vsprintf_sl(sl_u8_t buf, usz* _Nullable wrote_count, sl_u8_t fmt, va_list ap);
